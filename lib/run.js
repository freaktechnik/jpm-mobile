...
// Allow running with |jpm --dev|, like |jpm --nightly| etc.
// Local builds are named like fennec_USER.  Fish the user name from the environment.
binary_map['dev'] = 'fennec_' + process.env.USER;
...
function run(manifest, program) {
  ...
  return xpi(manifest, program).then(function (path) {
    ...
    then(function() {
      ...
      console.log(intent, FENNEC_REMOTE_PATH, program.adb);
